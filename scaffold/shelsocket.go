...
func (s *iopubSocket) sendDisplayData(data *DisplayData, parent *message, update bool) {
	...
	if update {
		if data.Transient["display_id"] == nil {
			log.Warning("update_display_data with no display_id")
		}
...
loop:
	for {
		...
		if isEINTR(err) {
			...
			log.Info("zmq.Poll was interrupted")
			continue
		}
		if err != nil {
			log.Errorf("Poll on %s socket failed: %v", s.name, err)
			continue
		}
		for _, p := range polled {
			...
			case s.socket:
				if err := s.handleMessages(); err != nil {
					log.Errorf("Failed to handle a message on %s socket: %v", s.name, err)
				}
			case s.resultPull:
				err := s.handleResultPull()
				if err == errLoopEnd {
					log.Infof("Exiting polling loop for %s", s.name)
					break loop
				}
				if err != nil {
					log.Infof("Failed to handle a message on the result socket of %s: %v", s.name, err)
				}
			default:
...
func (s *shellSocket) handleMessages() error {
	...
	case "kernel_info_request":
		if err := s.sendKernelInfo(&msg); err != nil {
			log.Errorf("Failed to handle kernel_info_request: %v", err)
		}
	case "shutdown_request":
		log.Info("received shutdown_request.")
		s.cancelCtx()
	...
	default:
		log.Warningf("Unsupported MsgType in %s: %q", s.name, typ)
	}
	return nil
}
